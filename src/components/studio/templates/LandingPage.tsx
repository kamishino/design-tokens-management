...
export const LandingPage = ({ data }: { data: StudioMockData }) => {
  return (
    <Box bg="white" minH="100vh" fontFamily="var(--fontFamilyBase)" color="gray.800">
      {/* 1. Sticky Navbar */}
      <Box 
          ...
          <HStack justify="space-between">
            <HStack gap={8}>
              <HStack gap={2}>
                ...
                <Text fontWeight="var(--fontWeightExtrabold)" fontSize="lg" letterSpacing="tight">
                  {data.brand.name}
                </Text>
              </HStack>
              <HStack gap={6} display={{ base: 'none', md: 'flex' }}>
                {['Features', 'Pricing', 'Testimonials', 'Docs'].map(item => (
                  <Text key={item} fontSize="sm" fontWeight="medium" color="gray.600" cursor="pointer" _hover={{ color: "var(--brandPrimary)" }}>
                    {item}
                  </Text>
                ))}
              </HStack>
            ...
            <HStack gap={4}>
              <Button variant="ghost" size="sm" color="gray.600">Log in</Button>
        ...
        <Container maxW="container.xl">
          <VStack gap={16}>
            <VStack gap={4} textAlign="center">
              <Heading size="2xl" letterSpacing="tight">Everything you need to scale</Heading>
              <Text color="gray.500" maxW="2xl">Our platform provides the tools to manage tokens from initial concept to global distribution.</Text>
            </VStack>
            ...
            <Tabs.Root defaultValue="sync" variant="enclosed" w="full" colorPalette="blue">
              <Tabs.List justifyContent="center" border="none" gap={4} mb={12}>
                <Tabs.Trigger value="sync" px={8} py={3} borderRadius="var(--radius2)" fontSize="sm" fontWeight="bold">
                  <LuZap style={{ marginRight: '8px' }} /> Real-time Sync
                </Tabs.Trigger>
                <Tabs.Trigger value="security" px={8} py={3} borderRadius="var(--radius2)" fontSize="sm" fontWeight="bold">
                  <LuShield style={{ marginRight: '8px' }} /> Enterprise Security
                </Tabs.Trigger>
                <Tabs.Trigger value="global" px={8} py={3} borderRadius="var(--radius2)" fontSize="sm" fontWeight="bold">
              ...
              <Tabs.Content value="sync" p={0}>
                <SimpleGrid columns={{ base: 1, md: 2 }} gap={12} alignItems="center">
                  <Box bg="gray.100" h="400px" borderRadius="var(--radius4)" position="relative" overflow="hidden">
                    <Box position="absolute" top="10%" left="10%" right="10%" bottom="10%" bg="white" borderRadius="var(--radius3)" boxShadow="2xl" p={8}>
                       <VStack align="start" gap={4}>
                         <Box w="40%" h="12px" bg="blue.100" borderRadius="full" />
                         <Box w="90%" h="12px" bg="gray.100" borderRadius="full" />
                         <Box w="70%" h="12px" bg="gray.100" borderRadius="full" />
                         <Separator my={4} />
                         <HStack gap={4}>
                           <Box w="32px" h="32px" bg="var(--brandPrimary)" borderRadius="var(--radius1)" />
                           <VStack align="start" gap={1}>
                             <Box w="60px" h="8px" bg="gray.200" borderRadius="full" />
                             <Box w="40px" h="8px" bg="gray.100" borderRadius="full" />
                           </VStack>
                         </HStack>
                       </VStack>
                    </Box>
                  ...
                  <VStack align="start" gap={6}>
                    <Heading size="xl" letterSpacing="tight">Instant preview in your browser</Heading>
                    ...
                    <List.Root gap={3} variant="plain">
                      <List.Item display="flex" alignItems="center" gap={3}>
                        <Icon as={LuCheck} color="green.500" /> <Text fontSize="sm">Automatic Style Dictionary builds</Text>
                      </List.Item>
                      <List.Item display="flex" alignItems="center" gap={3}>
                        <Icon as={LuCheck} color="green.500" /> <Text fontSize="sm">Hot Module Replacement for CSS</Text>
                      </List.Item>
                    </List.Root>
                  </VStack>
                </SimpleGrid>
              ...
              <Tabs.Content value="security" p={10} textAlign="center" bg="gray.50" borderRadius="var(--radius4)">
                <Heading size="lg">Enterprise-grade protection for your brand assets.</Heading>
              </Tabs.Content>
              <Tabs.Content value="global" p={10} textAlign="center" bg="gray.50" borderRadius="var(--radius4)">
                <Heading size="lg">Distribute tokens via NPM, CDN, or direct API integration.</Heading>
              </Tabs.Content>
            </Tabs.Root>
          </VStack>
        </Container>
                ...
                >
                  {tier.isPopular && (
                    <Float placement="top-center" translateY="-50%">
                      <Badge bg="var(--brandPrimary)" color="white" borderRadius="full" px={4} py={1}>Most Popular</Badge>
                    </Float>
                  )}
          ...
          <SimpleGrid columns={{ base: 1, md: 3 }} gap={8}>
            {data.testimonials.map((t, i) => (
              ...
              >
                ...
                <Text fontSize="md" color="gray.600" fontStyle="italic">"{t.content}"</Text>
                <HStack gap={4}>
                  <Image src={t.avatar} w={10} h={10} borderRadius="full" />
                  <VStack align="start" gap={0}>
                    <Text fontWeight="bold" fontSize="sm">{t.user}</Text>
          ...
          <SimpleGrid columns={{ base: 2, md: 5 }} gap={12}>
            <VStack align="start" gap={6} gridColumn={{ base: "span 2", md: "span 2" }}>
              <HStack gap={2}>
                ...
                <Text fontWeight="var(--fontWeightExtrabold)" fontSize="lg">{data.brand.name}</Text>
              </HStack>
              ...
              <VStack key={cat} align="start" gap={4}>
                ...
                <VStack align="start" gap={2}>
                  {['Features', 'Pricing', 'Security', 'Enterprise'].map(item => (
                    <Text key={item} fontSize="sm" color="gray.600" cursor="pointer" _hover={{ color: "var(--brandPrimary)" }}>{item}</Text>
                  ))}
                </VStack>
              </VStack>
            ))}
          </SimpleGrid>
          <Separator my={16} borderColor="gray.200" />
