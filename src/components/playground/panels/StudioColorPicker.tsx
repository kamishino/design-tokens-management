...
import { useState, useMemo, memo, useEffect, useCallback, type RefObject } from 'react';
import { 
  isOutofGamut,
  getContrastMetrics 
} from '../../../utils/colors';
...
export const StudioColorPicker = memo(({ color, onChange, label, variant = 'compact', containerRef }: StudioColorPickerProps) => {
  ...
  // --- CONTENT ---
  const Content = (
    <HStack p={0} gap={0} align="stretch" w={isExpanded ? "full" : "560px"} bg="white" borderRadius="md" boxShadow={isExpanded ? "sm" : "none"} border={isExpanded ? "1px solid" : "none"} borderColor="gray.100" overflow="hidden">
      {/* Left Column: Primary Picker */}
      <VStack p={4} w={isExpanded ? "300px" : "240px"} borderRight="1px solid" borderColor="gray.100" gap={4} align="stretch" bg="gray.50/30">
        <HStack justify="space-between">
          <Heading size="xs" textTransform="uppercase" color="gray.500" truncate>{label}</Heading>
          {outOfGamut && <Badge colorScheme="orange" size="xs">Gamut</Badge>}
        </HStack>
        
        <HexColorPicker color={color} onChange={handleColorChange} style={{ width: '100%', height: '140px' }} />
        ...
        <VStack align="stretch" gap={2}>
          <HStack gap={3}>
            ...
            <VStack align="start" flex={1} gap={0.5}>
              ...
              <HStack gap={1}>
                <Text fontSize="xs" fontWeight="bold" fontFamily="monospace">{contrast.wcag.toFixed(1)}</Text>
                <Badge colorPalette={contrast.isAccessible ? "green" : "red"} size="xs" variant="solid" fontSize="8px">
          ...
          <Box position="relative" w="full" h="28px" bg={color} borderRadius="md" border="1px solid rgba(0,0,0,0.1)">
            <Popover.Root positioning={{ placement: 'right-start', gutter: 12, flip: true, strategy: 'fixed' }}>
              ...
              <Portal container={containerRef}>
                <Popover.Positioner zIndex={2500}>
                  <Popover.Content w="240px" p={4} borderRadius="xl" boxShadow="2xl" border="none" maxH="60vh" overflowY="auto">
                      ...
                      {harmonySwatches.length > 0 && (
                        <VStack align="stretch" gap={2} pt={2} borderTop="1px solid" borderColor="gray.100">
                          ...
                          <HStack gap={2} flexWrap="wrap">
                            ...
                            <IconButton aria-label="Clear" size="xs" variant="ghost" onClick={() => setHarmonySwatches([])}>
                              <LuRotateCcw />
                            </IconButton>
                          </HStack>
                        </VStack>
      ...
      <Box flex={1} display="flex" flexDirection="column">
        <Tabs.Root defaultValue="oklch" size="sm" variant="subtle" flex={1} display="flex" flexDirection="column">
          <Tabs.List bg="gray.50" p={1} gap={1} borderBottom="1px solid" borderColor="gray.100">
            <Tabs.Trigger value="oklch" flex={1} fontWeight="bold" fontSize="10px">OKLCH</Tabs.Trigger>
            <Tabs.Trigger value="hsl" flex={1} fontWeight="bold" fontSize="10px">HSL</Tabs.Trigger>
            <Tabs.Trigger value="swatches" flex={1} fontWeight="bold" fontSize="10px">Swatches</Tabs.Trigger>
          </Tabs.List>
          ...
          <Box p={4} flex={1} overflowY="auto">
            ...
            <Tabs.Content value="swatches" p={0}>
              <VStack gap={3} align="stretch">
                <Input placeholder="Search tokens..." value={search} onChange={(e) => setSearch(e.target.value)} size="xs" borderRadius="md" />
  ...
  if (variant === 'button') {
    return (
      <Popover.Root positioning={{ placement: 'bottom-start', gutter: 4, flip: true, strategy: 'fixed' }} lazyMount unmountOnExit>
        <Popover.Trigger asChild>
          <Box 
            p={3} 
            className="group"
        ...
        <Portal container={containerRef}>
          <Popover.Positioner style={{ zIndex: '9999 !important' as any }}>
            <Popover.Content borderRadius="xl" boxShadow="2xl" border="none" w="auto" maxH="500px">
              {/* Force compact mode inside Popover */}
              <StudioColorPicker color={color} onChange={onChange} label={label} variant="compact" containerRef={containerRef} />
            </Popover.Content>
          </Popover.Positioner>
        </Portal>
      </Popover.Root>
